...
RULES = [
    ...
    ("press_summary", r"\bотчет о ходе процесса\b"),
    ...
    ("correspondence", r"\bпереписка\b"),
    ("letter", r"\bписьм[оа]\b|\bписьма\b|^[А-ЯЁ]\.[А-ЯЁ]\.\s+[А-ЯЁ][а-яё-]+\s+[–-]\s+[А-ЯЁ]\.[А-ЯЁ]\."),
    ("statement", r"\bзаявлени[ея]\b|\bзаявления\b"),
    ("session_transcript", r"\bстенограмма\b|\bфрагмент стенограммы\b|\bзаседани[ея]\b|\bпротокол общего закрытого собрания\b|\bфрагмент протокола общего закрытого собрания\b"),
    ...
    ("biographical_article", r"^(валентин ольберг|георгий борисович синани[- ]скалов)$"),
...
def classify(title):
    ...
    if matches:
        return matches[0], " | ".join(matches), "rule_title_v4"
...
report = []
report.append("ShowTrials document type v4 diagnosis")
report.append("")
